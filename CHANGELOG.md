...
### 中文
- **Windows 安装器在安装阶段预解包内置内核，首启不再付解包代价**：安装器的 `customInstall` 钩子用系统 `tar.exe` 把 `resources/kernel/kernel.tgz` 解到 `resources/kernel-staged/`，首启直接搬树激活（同盘重命名、跨盘复制）。实测同一棵 rc.2 运行时：内核安装从 **18.2 秒降到 0.3 秒**。tarball 的 sha512 在碰暂存树之前照验，暂存树缺失、半截或 manifest 不符全部回退到应用自己解包——预解包失败只是首启慢一点，不是装不上。仅 Windows（其他平台安装器没有 NSIS 钩子）。
- **配置备份补上迁移真正需要的两块，密钥携带从“拒绝”改为“告知”**：备份现在包含 home 层补丁（`~/.dsh/cordis.patch.yml`，用户手写提供商路由的家）和 dsh 凭据库（`.credentials.yaml`，API 密钥明文）——此前两者都不在包内，换机器等于重填所有钥匙。两块都按精确归档路径准入，凭据库覆盖前自动留副本。内容扫描不再拒导：命中改为收集 `{文件, 规则}` 警告，随导出响应经 `x-dsh-backup-warnings` 头返回，客户端提示“ZIP 内含明文密钥，只在本机保管或先加密再传输”；凭据库即使内容不命中任何规则也必定告警。恢复与覆盖确认的文案同步补上凭据库，冲突列表里 `credentials.yaml` 显示为「凭据库（API 密钥）」。
...
### English
- **The Windows installer pre-extracts the bundled kernel at install time, so the first launch pays no extraction**: the installer's `customInstall` hook unpacks `resources/kernel/kernel.tgz` into `resources/kernel-staged/` with the system `tar.exe`, and the first launch adopts the staged tree directly (a rename on one volume, a copy across volumes). Measured on the same rc.2 runtime: the kernel install went from **18.2 s to 0.3 s**. The tarball's sha512 is still verified before the stage is touched, and a missing, half-written or mismatching stage falls back to the app's own extraction — a failed pre-extraction is a slower first launch, never a broken install. Windows only (the other platforms' installers have no NSIS hook).
- **The config backup now carries the two things a migration actually needs, and key material moved from "refused" to "disclosed"**: the backup includes the home patch layer (`~/.dsh/cordis.patch.yml`, where users hand-write provider routes) and dsh's credential store (`.credentials.yaml`, API keys in plaintext) — both were absent before, so a new machine meant re-entering every key. Both are exact-path members, and the store is sidecarred before an overwrite. The content scan no longer refuses the export: hits are collected as `{file, rule}` warnings that ride the answer in an `x-dsh-backup-warnings` header, which the client turns into "the zip holds plaintext keys — keep it local or encrypt it"; the store is reported even when its content matches no rule. Restore and overwrite copy now names the store, and the conflict list shows `credentials.yaml` as "the credential store (API keys)".
...
- **rc.2 的 office kit 0.1.1 复验：外壳侧无需改动**。host 依赖升到 `^0.1.1`，但它只用 `import.meta.resolve` 推 `lib/cli.js` 路径、交给 skill-office 做存在性检查，不校验版本；垫片停在 0.1.0 照旧满足（入口按 payload 内 kit 自己的 manifest 解析，与版本解耦；runtime 自带 Node v24.18.0，满足 kit 的 `>=22.19.0`）。载荷的内容版本在 rc.1 → rc.2 之间**保持 0.1.1-py3.12.14 不变**（kit 0.1.1 + python 3.12.14 + 引擎，按发布流程的格暂格构建），所以已下载过该载荷的用户**不会**被要求重装；只有停在旧 0.1.0 载荷的用户需要下一次——这正是按内容版本寻址的设计目的（真机转换待启动确认时一并验）。
...
### 中文
- **账号与余额页可用，并且能在应用内打开「查询用量」「充值」**。这块能力一直在内核里（`dsh-base` 已挂载 `authorization` / `deepseek-account` / `credentials`），缺的只是一个开关：内核的账号界面第一行就是 `if (!('dshDesktop' in globalThis)) return`，而本壳**没有 preload**，于是账号设置页、登录对话框、余额卡、头像菜单**全部静默不注册**，且不报任何错。现在主窗口带一个 preload，暴露**恰好两个**东西：一个冻结的 `window.dshDesktop` 标记，和 `window.dshPlatform`（open/setBounds/close 三个操作，全部由主进程校验「本窗口主框架 + app 文档」后才响应）。**令牌不经过这个桥**：主进程持有账号会话，内嵌页自己的 preload 通过同步通道取，app preload 里另有断言禁止它碰凭证通道。内嵌用量/充值页跑在**独立非持久分区**里：拒绝一切权限、禁止下载、只对 Platform origin 注入部署头、重定向后剥离、跨源导航拦截、`window.open` 走系统浏览器。**实测**：真机登录成功、账号菜单显示头像与「退出登录」、点充值在应用内加载 `platform.deepseek.com/top_up`（标题「DeepSeek 开放平台」）
- **修掉内嵌页「操作未完成，请重试」的两个缺陷**（都是首次实现里的真错）。① 帧校验用了 `new URL(frameUrl).origin`，而**自定义 scheme 没有 registrable origin**——实测 `new URL('dsh-app://app/index.html').origin` 就是字符串 `"null"`，于是每一次请求都被自己的校验拒掉（日志 `rejected sender`）。改为按 **scheme + host** 比较（上游 `assertDesktopSender` 的同一做法），抽成纯函数并补了正反两组断言，其中一条**把回归本身钉成事实**。② 内核发布的账号会话**先于** host 的 `ready` 到达，而视图是在 `server.start()` **之后**创建的，`platformView?.setSession(...)` 把它**静默丢弃**（日志 `no account session`）。改为在 `server.start()` **之前**创建视图
- **内嵌页跟随应用主题**：实测两种做法，只有一种有效——URL 上的 `?theme=light|dark` 对用量/充值页**无效**（那是登录页的参数：`?theme=light` 在深色系统偏好下页面照样 `rgb(21,21,23)`），页面实际跟随 `prefers-color-scheme`。所以改为打开视图前把 `nativeTheme.themeSource` 设成应用解析出的主题、关闭时**恢复原值**（该开关是进程级的，不能永久改）。主题值来自与启动画面同一条链路，所以「浅色 / 深色 / 跟随系统」都正确。**顺带修掉一个更早的缺陷**：那条链路原本**只在启动时算一次**，`currentTheme` 此后永不更新——这正是「切成深色后内嵌页仍是浅色」的原因；现在每次问都重新解析（读 profile patch 的三个小文件）。**真机双向验证**：深色偏好下 `prefersDark:true / rgb(21,21,23)`，运行中改成浅色后重新打开 `prefersDark:false / rgb(255,255,255)`，无需重启
- **设置导航调整**：「用量统计」从「维护」里的一个页签**提为独立行**（order 21）——留在维护的是系统级页面（预设包、诊断，回答"这套装是否健康"），而用量是用户自己的数据视图，藏在页签后难以发现（实测在侧栏找它找不到）；且它与「账号与余额」相邻才读得通（账号页是服务端余额，用量页是本地日志折算）。「维护」**改名为「维护设置」**（与「通用设置」同族，四字）并**移到套件最后**（order 22 → 24，会话记忆/归档各前移一位到 22/23）：它此前夹在两个数据视图中间，读起来是「数据 → 系统 → 数据」的来回跳；设置里的「维护」本就该是最后一项。**真机确认**侧栏为 `… Agent 预设 → 用量统计 → 会话记忆 → 会话归档 → 维护设置 → 手机连接`

### English
- **The account section works, and 查询用量 / 充值 open INSIDE the app.** The capability was always in the kernel (`dsh-base` mounts `authorization` / `deepseek-account` / `credentials`); what was missing was one switch. The kernel's account UI opens with `if (!('dshDesktop' in globalThis)) return`, and this shell had **no preload** — so the account settings page, the sign-in dialog, the balance card and the avatar menu were **silently never registered**, with no error anywhere. The main window now carries a preload exposing **exactly two** things: a frozen `window.dshDesktop` marker, and `window.dshPlatform` (open / setBounds / close, each answered by the main process only for this window's top frame on the app document). **No credential crosses that bridge**: the main process holds the account session and the embedded page's own preload fetches it over a synchronous channel; the app preload has assertions forbidding it from touching the credential channel. The embedded usage / top-up page runs in its **own non-persistent partition**: every permission is refused, downloads are off, deployment headers are injected only for the Platform origin and stripped after a redirect, off-origin navigation is prevented, and `window.open` goes to the system browser. **Measured**: real sign-in worked, the account menu shows the avatar and 退出登录, and 充值 loaded `platform.deepseek.com/top_up` inside the app (title 「DeepSeek 开放平台」)
- **Fix two real defects behind the embedded page's "操作未完成，请重试"**. ① The frame check compared `new URL(frameUrl).origin`, but a **custom scheme has no registrable origin** — measured, `new URL('dsh-app://app/index.html').origin` is the string `"null"`, so every request was refused by its own guard (log: `rejected sender`). It now compares **scheme + host** (the same thing upstream's `assertDesktopSender` does), extracted into a pure function with both directions asserted — one test pins the regression itself as a fact. ② The kernel publishes its account session **before** the host's `ready`, while the view was created **after** `server.start()`, so `platformView?.setSession(...)` **dropped it silently** (log: `no account session`). The view is now created **before** `server.start()`
- **The embedded page follows the app's theme**: of two candidate approaches, only one works — measured, a `?theme=light|dark` parameter does **nothing** for the usage / top-up pages (that is the LOGIN page's lever: `?theme=light` under a dark OS preference still painted `rgb(21,21,23)`), while the page does follow `prefers-color-scheme`. So the fix sets `nativeTheme.themeSource` to the app's resolved mode before the view opens, and **restores the previous value on close** (the setting is process-wide, so it must not be changed for good). The value comes from the same chain the splash uses, so 浅色 / 深色 / 跟随系统 all resolve correctly. **An older defect fell out of this**: that chain was computed **once at startup** and `currentTheme` never changed afterwards — which is exactly why switching to dark left the embedded page light. It is now re-resolved on every call (three small files in the profile patch). **Verified both ways on a real app**: under the dark preference `prefersDark:true / rgb(21,21,23)`; after changing to light at runtime and reopening, `prefersDark:false / rgb(255,255,255)` — no restart
- **Settings navigation moved**: 用量统计 went from a tab inside 维护 to **its own rail row** (order 21) — what stays in 维护 are the system-level pages (preset packages, diagnostics: "is this install healthy?"), while usage is the user's own data view and was hard to find behind a tab (measured: a search for it in the rail came up empty), and it only reads correctly next to 账号与余额 (that page shows the server's balance, this one the local logs). 维护 was **renamed 维护设置** (the 通用设置 family, four characters) and **moved to the end of the suite band** (order 22 → 24, with session memory/archives moving up to 22/23): it used to sit between two data views, so the rail read "data → system → data", and upkeep belongs last in a settings list. **Confirmed on a real app**: `… Agent 预设 → 用量统计 → 会话记忆 → 会话归档 → 维护设置 → 手机连接`
...
- **同时恢复：代理注入照旧，两个能力都在**（默认启动、代理开着、无任何环境变量覆盖，实测）：模型列表 `ok:true`、**27 个模型**；`web_fetch` 经真 provider 类走代理取回 `https://example.com/` → `200 / 15B`、`https://cn.bing.com/` → `200 / 15B`（路由日志 `proxied via http://127.0.0.1:7897`）。`DSH_APP_PROXY_INJECT=off` 保留为逃生口（不注入），但不再是默认路径的一部分

### English
- **Fix the real cause of "every provider fails to list its models while a proxy is up", and withdraw the previous release's wrong remedy**: both capabilities worked on the 0.1.6 line and every model list broke on 0.1.7 as soon as a proxy was present, and the whole difference was the **`undici` version the runtime carried** — **8.10.2** on 0.1.6, **8.11.0** on 0.1.7. Between those two releases **exactly one line** changed (`lib/dispatcher/dispatcher1-wrapper.js`, `dispatch`): 8.10.2 read `if (opts.allowH2 !== false)` and downgraded every request to HTTP/1.1, while 8.11.0 reads `if (opts.upgrade && opts.allowH2 !== false)` and downgrades WebSocket upgrades only. So that a bare `fetch` also goes through the proxy, the kernel installs its own undici's compatibility wrapper onto the global slot Node's BUILT-IN `fetch` reads — and under 8.11.0 an ordinary GET is then dispatched over HTTP/2 into a v1 handler, which **drops every response header and the body decoding**. `llm-pi-ai`'s model discovery calls bare `fetch`, which is why only the model list broke (the kernel's own `web_fetch` uses its own undici's `fetch` and was never affected). **Measured**: one rc.1 runtime tree, one Node, only the undici directory swapped — 8.11.0 gave `392 B / 0 headers / not JSON`, 8.10.2 gave `2172 B / 16 headers / JSON`. **The fix** pins `undici` to 8.10.2 at the runtime-assembly layer (the install input's `overrides` in `scripts/build-runtime.mjs`), so it travels with the runtime to every platform. **The previous release's "a fake-IP network gets no proxy" rule is deleted outright** — it was a trade made under the wrong root cause, buying the model list with `web_fetch`, when the two can both hold
- **Both capabilities are back, with injection unchanged** (default launch, proxy up, no environment override; measured): the model list answers `ok:true` with **27 models**, and `web_fetch` through the real provider class over the proxy returns `https://example.com/` → `200 / 15 B` and `https://cn.bing.com/` → `200 / 15 B` (route log: `proxied via http://127.0.0.1:7897`). `DSH_APP_PROXY_INJECT=off` stays as the opt-out, no longer part of the default path

## [v0.13.3] - 2026-09-24

### 中文
- **修复「设置写入一律被拒」的根因：数据目录的两种拼写让内核把同一个模块加载了两份**（症状：内测声明点确认永远失败、切换预设报错、模型商列表消失、高级配置拉不到模型）。0.1.7 起外壳要把数据目录改名为品牌拼写 `DSH APP`，判据却是 `existsSync(旧名) && !existsSync(新名)`——在 Windows 与默认 macOS 卷上这两个名字**是同一个目录**，于是改名从未发生：磁盘上一直是 `DSH App`，而外壳交给内核子进程的是字面量 `DSH APP`。Node 的 ESM 缓存按 URL 字符串认模块，`@deepseek-ai/dsh-app-boot` 因此被实例化**两次**（探针实测：`…/DSH%20APP/kernel/…` 与 `…/DSH%20App/kernel/…` 各一次）；而该包用一个模块级 WeakMap 记住「哪个 Context 挂上了根 Include」，挂载写进实例 A、配置编辑器查的是实例 B，查不到就抛 `profile reload requires the root Include entry`。现在名字对齐交给新的 `src/main/user-data.ts`：不是品牌拼写就两步改名（只改大小写的改名必须分两步），改不动就**采用磁盘上的拼写**并把后果写在日志里。**实测**：目录改名后重启同一个安装版，模块加载从 2 次变 1 次、每次 reconcile 的 `has` 从 false 变 true，同一个写入 RPC 从 `settings/rejected` 变 `ok:true`；真机复验模型页列出 **45 条 provider 路由**（含 7 条「手写」路由），首屏不再弹内测声明
- **被旧判据注释掉的行现在会自己回来**：上一版修好的是误判本身，而那只防将来——文件里已经躺着的死文本仍被逐字带下去（实测：同一行被注释、手工恢复、又被仍在运行的旧构建注释掉，来回两次）。现在外壳在过滤之前先读 `NOT LOADED` 标记后的块，解除注释后**再判一次**：只有块里每个名字都能解析、且解除后的文本仍然读得出是行（`- ` 起首、缩进续行或空行）才留下。后一条不是洁癖——标记无法区分「被注释的代码」与「用户写在行内第一列的注释」，直接删掉 `# ` 会把散文塞进城加器等待 YAML 的位置；段落边界也按两种真实情况收住：下一个 `NOT LOADED` 标记（两行死文本相邻时各判各的），以及文件自己的 `# @@dsh-app-rows:` 标记（它紧跟每一段最后一条死行，当成代码就会把 `@@dsh-app-rows:home` 塞进行里）。根测试 395 项 / 394 通过 / 0 失败 / 1 跳过（新增 9 项），并有一条会真写文件的组合测试盯住整条路径
- **修复「每个 provider 都取不到模型」**（主人报的）：真应用里问任何一条已配置的路由都回 `llm/model-discovery-rejected … did not answer with JSON`——一句话的含义是「**2XX 但 body 不是 JSON**」，不是 401、也不是网络不通（端点本身用同一个 key 直连与走代理都是 `200 / 2104B / 合法 JSON`，宿主也确实解析出了凭据：本地回显服务器抓到 `authorization: Bearer …(len 55)`，同一调用回 `ok:true` + 两个模型）。**根因在上游的代理支持与 Node 内建 `fetch` 的配对**：内核为了让裸 `fetch` 也走代理，把自家 `undici@8.11.0` 的 dispatcher 装进全局槽，而那份 undici 在 import 时会给 Node 内建 `fetch` 读的旧符号装一层兼容包装——内建 `fetch` 读到的响应**头全丢、body 不解压**（运行时自带 Node 22.23.2 与换成 Node 24.18.0 表现完全一致；同一进程里改用该包自己的 `fetch` 就一切正常：200 / 2080B / 14 头 / JSON）。诱因是外壳注入的代理环境变量，而这台机器是 **TUN/fake-IP**（`cpa.geluman.cn → 198.18.1.134`、本机 `198.18.0.1`）：对它来说代理 URL 只买到 `web_fetch` 的地址校验（内核在解析结果落在保留段时，**只有看得见代理才跳过校验**），代价却是模型列表。所以新增规则：**占位地址的网络不注入代理**（`decideProxyOffer`，含双向逃生口 `DSH_APP_PROXY_INJECT=on|off`；用户显式导出的代理环境仍然优先），日志写明原因与代价。**默认启动真跑复验**：同一个 RPC 从 `ok:false` 变 `ok:true`、回 **26 个模型**。**已知代价**：这台机器的 `web_fetch` 回到被地址校验拒绝的状态（注入功能存在之前就是这样），要那边的能力就用变量换回来；两者在当前内核线上不能同时成立——上游报告已写进回归文档 §12.13.4

### English
- **Fix the cause of "every settings write is refused": two spellings of the data directory made the kernel load one module twice** (symptoms: the first-run notice could never be acknowledged, switching an agent preset reported an error, the provider list disappeared, and the advanced model configuration could not fetch models). Since 0.1.7 the shell renames its data directory to the brand spelling `DSH APP`, but the test was `existsSync(old) && !existsSync(new)` — and on Windows and default macOS volumes **those are the same directory**, so the rename never happened: the disk kept `DSH App` while the shell handed the kernel child the literal `DSH APP`. Node's ESM cache keys modules by URL string, so `@deepseek-ai/dsh-app-boot` was instantiated **twice** (measured with a probe: once under `…/DSH%20APP/kernel/…`, once under `…/DSH%20App/kernel/…`), and that package keeps a module-level WeakMap of which Context booted the root Include: the mount wrote into instance A, the configuration editor asked instance B, found nothing and threw `profile reload requires the root Include entry`. One spelling is now decided by the new `src/main/user-data.ts`: a directory that is not already the brand spelling is renamed in two steps (a case-only rename needs two), and when the rename cannot be made the **on-disk spelling is adopted** with the consequence written to the log. **Measured**: after the rename and a restart of the same installed build, module loads went from two to one, every reconcile's `has` from false to true, and the same write RPC from `settings/rejected` to `ok:true`; on a real boot the models page lists **45 provider routes** (seven of them hand-written) and the first-run notice does not appear
- **Rows an older build commented out now come back on their own**: the previous release fixed the misjudgement itself, which only prevents new comments — the dead text already in the file travelled forward verbatim on every start (measured: the same row was commented, restored by hand, and commented again by the still-installed old build, twice). The shell now reads the block behind a `NOT LOADED` marker before filtering and **judges it again** after un-commenting: it is kept only when every name in it resolves and what comes out still reads as loader rows (`- ` at column zero, an indented continuation, or blank). That second rule is not fussiness — the marker cannot tell commented code from a comment the user wrote in the row's first column, and stripping `# ` would put bare prose where the loader expects YAML. Two boundaries are taken from real files: the next `NOT LOADED` marker (two dead rows side by side are judged one at a time) and the file's own `# @@dsh-app-rows:` markers (one follows the last dead row of every section, and taking it for code puts `@@dsh-app-rows:home` inside a row). Root suite 395 tests / 394 pass / 0 fail / 1 skipped (nine new), with a composition test that writes a real file guarding the whole path
- **Fix "not one provider can fetch its models"** (reported): in the real app, interrogating any configured route answered `llm/model-discovery-rejected … did not answer with JSON` — and that sentence means "**2xx, but the body is not JSON**", not a 401 and not an unreachable host (the endpoint answers `200 / 2104 B / valid JSON` with the same key direct and through the proxy, and the host does resolve the credential: a local echo server received `authorization: Bearer …(len 55)` and the same call returned `ok:true` with two models). **The cause is upstream's proxy support meeting Node's built-in `fetch`**: so that a bare `fetch` also goes through the proxy, the kernel installs its own `undici@8.11.0` dispatcher into the global slot, and at import that undici wraps the legacy symbol Node's built-in `fetch` reads — whose responses then arrive with **every header gone and the body undecompressed** (identical on the runtime's Node 22.23.2 and on Node 24.18.0; in the same process the package's own `fetch` is fine: 200 / 2080 B / 14 headers / JSON). What triggers it is the proxy environment the shell injects, and this machine is **TUN/fake-IP** (`cpa.geluman.cn → 198.18.1.134`, its own address `198.18.0.1`): there the URL buys only `web_fetch`'s address validation (which the kernel skips only when a proxy is visible) and costs the model list. Hence the new rule: **a machine that answers with placeholder addresses is offered no proxy** (`decideProxyOffer`, with `DSH_APP_PROXY_INJECT=on|off` as the two-way escape hatch; a proxy the user exported themselves still wins), and the log line states both the reason and the cost. **Verified on a default launch**: the same RPC went from `ok:false` to `ok:true` with **26 models**. **Known cost**: on that machine `web_fetch` returns to being refused by address validation, exactly as it was before the injection feature existed — flip it back with the variable if you prefer; the two cannot both hold on this kernel line, and the upstream report is in the regression document §12.13.4

## [v0.13.2] - 2026-09-23

### 中文
- **诊断页的组合静态检查现在区分「谁的行」**：原来只有「我们的 / 其他」两档，而 0.1.7 的预设迁移把用户自己的预设写成了一行 `@deepseek-ai/dsh-agent-preset`——按模块前缀归属，它会混进「其他」，让人怀疑是自己装的插件出了问题。现在分三档：**我们的行 / 我们迁移的预设行 / 其他**，徽章与摘要给出三者的条数（实测：`我们的行没有问题（另有 12 条在别的行上）`，其中 1 条标着「我们迁移的预设」）。另外记一笔事实：内核自带的四个预设行在任何 profile（含全新 profile）上都会报同一条 `unrecognized Loader tree carrier`——那是内核静态检查只认原生 `cordis:group`/`cordis:include` 载体所致，不是损坏
- **内核线跟到 0.1.7-rc.1**（跟随 spec 现为 `^0.1.7-rc.1`；`latest` 这个 tag 仍停在 0.1.5-rc.3，跟这条线必须走 `next`）。相对 alpha.2 是 156 个 commit，**已导入的 API 面零破坏**：约 250 条内核 import 逐条比对，无一改名或改签名，有真实改动的五个包（`ui-primitives`、`ui-conversation`、`api/remotes`、`ui-theme`、`office-to-pdf`）相对我们的使用点都是纯增量。**要改的在 Office**：rc.1 的桌面宿主在启动时从宿主包的 manifest 推出 `lib/cli.js`，连同 `<primaryRuntime>/dependencies/node/bin/node` 一起交给 `skill-office`，后者逐个 `statSync`，缺一个就抛错——而这个抛错发生在宿主自己的 `main()` 里，**宿主进程直接退出、应用起不来**。我们两处都对不上：垫片目录没有 `lib/cli.js`；载荷未安装时叶子是空的，而 Linux 格本来就不产 Python 集，等于永久起不来。修法是垫片补 `lib/cli.js`（转发到载荷里真实的 kit CLI，载荷缺失时给出可操作拒绝），叶子在没有 Python 集时改为承载运行时自带的 Node（硬链，跨卷回退为复制），并同步 `stageOfficeKitShim` 的必需文件断言（原来只断言 `package.json` 与 `index.js`）。载荷要按 kit **0.1.0** 重建，内容版本随组装树里真实 kit 的版本从 `0.0.1` 变 `0.1.0`，代码不用改。**插件 manifest 的 dsh 范围刻意不动**——`^0.1.7-alpha.2` 覆盖整条 0.1.7 线，而 `^0.1.7-rc.1` 不匹配 alpha.2，内核回滚时门禁会把 16 行全部就地禁用
- **修复 runtime 构建在 npm 12 上必然失败**：`npm pack --json` 从 npm 12 起返回以包名为键的**对象**，不再是数组，而 `packDesktopHost` 按 `packed[0].filename` 读，于是报「npm pack produced no file name for @deepseek-ai/dsh-desktop-host」——这句话完全没提真正的原因。现在两种形状都接受。CI 用 Node 22 自带的 npm 10，所以这个缺陷只在本地较新的 npm 上暴露
- **插件市场给每一行标注归属**：套件插件标「套件」（由桌面壳管理），其余一律标「第三方」并写明"由社区作者维护：本套件只负责把它装进/移出 profile"。这条不是装饰——第三方插件的客户端半边若是针对别的内核线构建的，会渲染成错误卡（`dshmarket` 就是），没有这个标注时看起来像我们的问题
- **运行时随包 pnpm：插件的安装/更新不再依赖你机器上装过 pnpm**。内核所有的包操作都是子进程，而「CLI 那条路」是按**名字**（`pnpm`）从 `PATH` 上找的——所以在一台没装 pnpm 的机器上，插件管理页、内核自带的插件页、以及启动时的 profile 修复**全部**会在 spawn 那一步失败（实测：有 pnpm 时 `dsh plugin … add` 正常装好，没有时是 cmd 的「不是内部或外部命令」）。现在运行时自带**锁定版本**的 pnpm 11.7.0（与构建 runtime、写 profile lockfile 的版本同一个；来源与校验沿用办公组件那份 pin：npmmirror 优先、sha512 校验），外壳把它接到两处：**子进程 `PATH` 的最前面**（CLI 按名字找得到，且优先于用户自己的版本），以及**桌面宿主的包管理器参数位**（内核自带的插件页走这条）。**实测**：把 `PATH` 里所有 pnpm 目录删掉后——同一个 `dsh plugin … add` 不带随包 shim 时失败、带上就装好（`Done in 614ms using pnpm v11.7.0`）；在**真应用**里从插件市场装 `dsh-better-edit` 返回 `installed: true, 0.8.2`、pnpm 输出 `Packages: +17 … Done in 2s`。层划分同步更新（`runtime/pnpm` 与 `runtime/node` 同层，内容寻址，重新组装逐字节校验通过）
- **修复一行 scoped id 让整个 profile patch 解析失败**（市场托管禁用块）：插件市场把条目 id 写进 patch 的托管块时，若 id 以 `@` 开头（例如 `@deepseek-ai/dsh-tool-session-query`）就直接裸写——而 `@` 在 YAML 里是**保留字符、不能起一个普通标量**。后果不是"读歪了"，是**整份 `cordis.patch.yml` 解析不了**（`bad indentation of a mapping entry`），内核子进程当场退出，**装好的应用与 dev 两套都起不来**，直到手工给那一行加上引号。现在这种 id 一律加引号写入（读取端本来就剥引号，所以开关仍认得自己那行），市场版本 `0.2.0 → 0.2.1`，并补了一条测试钉住引号、`disabledIdsOf` 往返与删除路径
- **开发版启动：装的内核与仓库跟随线不一致时拒绝启动**：原来只打一行警告然后照起，而实测的后果是**两道内核线互相污染同一个 profile**——旧内核启动时把自己的包投影进 profile（`node_modules/@scope/*` → `.dsh-module-fallback`），此后跟随新线的每次启动都会加载**那批旧包**（`settings … this.load is not a function`、`configForms` 缺失、7 条客户端条目 pending）。现在会读出已装内核声明的版本，与仓库跟随线不一致就停下，并把两条线号与两条出路（先构建一份 runtime，或用 `DSH_APP_DEV_KERNEL=<dir>` 指定）都写出来；同线安装仍照常回退
- **内核线跟到 0.1.7-alpha.2**（同线的补丁位；跟随 spec 现为 `^0.1.7-alpha.2`，17 个 manifest / 351 处同时改）。这一版把 vendor 包抬到同一次发布的版本（cordis **4.0.4**、schemastery **3.18.4**、loader 1.0.5、group 1.0.4、include 1.0.9、timer 1.1.6、cosmokit 1.8.5…）。**两处行为要你知道**：① 内核子进程现在对**任何未捕获的 rejection / 异常**一律失败退出（`fail-loud`），插件里一次异步抛错不会再被吞掉；② 「首次安装插件且未配置源」会先去找最优可访问的 npm 源（仍可手动选源与私有源）。**我们这侧无需改代码**——根测试 373/0、16 个插件 1226/0、`check:graph` 干净。待跟的项目与复验清单见 `docs/kernel-0.1.7-alpha.1-regression.md` §12
...
- **开发版启动改为跟随本地 runtime**：`npm run dev` 不再探测并直接跑 `../deepseek-harness` 源码树（源码没构建就会启动上一版产物——本机实测检出落后三次构建、还带着 0.1.6 的符号）。选择顺序是：`DSH_APP_DEV_KERNEL` 指定的树 → `DSH_APP_DEV_RUNTIME` 指名的实时源码 → **本仓自己构建、且声明着跟随线的 runtime 树**（`scratch/<名字>/runtime`，按 manifest 判、不是按目录猜）→ 已安装的运行时（**只在同线时才回退**，见本列表第二条）
...
### English
- **The diagnostics page now says WHICH side each composition finding is on**: it used to offer two buckets (ours / others), and the 0.1.7 preset migration writes a user preset as an `@deepseek-ai/dsh-agent-preset` row — attribution by module prefix filed that under "others", leaving the reader to wonder whether a plugin they installed was at fault. Three buckets now: **our rows / a preset row this app migrated / others**, with the counts in both the badge and the summary (measured: `No findings on our rows (12 elsewhere)`, one of them labelled "our migrated preset"). Recorded alongside: the kernel's own four preset rows report the same `unrecognized Loader tree carrier` on ANY profile, a fresh one included — its static checker accepts only the native `cordis:group`/`cordis:include` carriers, so this is not damage
- **Kernel line moved to 0.1.7-rc.1** (the followed spec is now `^0.1.7-rc.1`; the `latest` tag still sits at 0.1.5-rc.3, so following this line must go through `next`). That is 156 commits past alpha.2 with **no break in the imported API surface**: about 250 kernel imports were compared one by one and none was renamed or re-signed, and the five packages with real changes (`ui-primitives`, `ui-conversation`, `api/remotes`, `ui-theme`, `office-to-pdf`) are additive at every point we use. **The work is in Office**: the rc.1 desktop host derives `lib/cli.js` from its own package manifest at boot and hands that, together with `<primaryRuntime>/dependencies/node/bin/node`, to `skill-office`, which `statSync`s each of them — and that throw happens inside the host's own `main()`, so **the host process exits and the app never comes up**. We missed both halves: the shim directory carried no `lib/cli.js`, and the leaf was empty whenever no payload was installed (Linux cells stage no Python set at all, so they would never have started). The fix adds `lib/cli.js` to the shim (forwarding to the real kit CLI inside the payload, with an actionable refusal while it is absent), has the leaf carry the runtime's own Node when there is no Python set (hard-linked, copied when the volumes differ), and extends `stageOfficeKitShim`'s required-file assertion, which only knew `package.json` and `index.js`. The payload must be rebuilt against kit **0.1.0** — its content version follows the real kit in the assembled tree from `0.0.1` to `0.1.0`, so no code changes for that. **The plugins' dsh ranges are deliberately left alone**: `^0.1.7-alpha.2` spans the whole 0.1.7 line, while `^0.1.7-rc.1` does not match alpha.2, so a kernel rollback would have the compatibility gate disable all sixteen rows
- **Fix the runtime build failing on npm 12**: since npm 12, `npm pack --json` answers an object keyed by package name rather than an array, and `packDesktopHost` read `packed[0].filename` — so it reported "npm pack produced no file name for @deepseek-ai/dsh-desktop-host", a message that says nothing about the cause. Both shapes are accepted now. CI runs npm 10 with Node 22, which is why the defect only surfaced on a newer local npm
- **The plugin market now labels every row with its owner**: suite plugins read 套件 (`Suite` — managed by the desktop shell) and every other row reads 第三方 (`Third-party`) with a note that its author maintains it and the suite only installs or removes it. That is not decoration — a third-party plugin whose client half was built against another kernel line renders as an error card (`dshmarket` is one), and without the badge that reads as ours
- **The runtime now ships pnpm, so installing or updating a plugin no longer needs one on your machine**: every package operation the kernel performs is a child process, and the CLI path resolves the command by NAME (`pnpm`) through `PATH` — so on a machine without pnpm the plugin manager page, the kernel's own plugin page and the boot-time profile repair all fail at the spawn (measured: `dsh plugin … add` installs when a pnpm is present and answers cmd's "not recognized" without one). The runtime carries a **pinned** pnpm 11.7.0 — the same version that assembles the runtime and writes the profile lockfiles, obtained and verified through the pin the office payload already uses (npmmirror first, sha512 checked) — and the shell wires it in twice: at the **front of the child's `PATH`** (so the CLI resolves it by name, ahead of any user install) and in the **desktop host's package-manager argument slots** (what the kernel's own plugin page uses). **Measured**: with every pnpm directory removed from `PATH`, the same `dsh plugin … add` failed without the bundled shim and installed with it (`Done in 614ms using pnpm v11.7.0`); in the real app the market installed `dsh-better-edit` and answered `installed: true, 0.8.2` with pnpm reporting `Packages: +17 … Done in 2s`. The layer split follows (`runtime/pnpm` rides with `runtime/node`, content-addressed, and the re-assembly check reproduces the tree byte for byte)
- **Fix one scoped id making the whole profile patch unparsable** (the market's managed-disable block): when the plugin market wrote an entry id into the patch's managed block, an id starting with `@` (for instance `@deepseek-ai/dsh-tool-session-query`) went in bare — and `@` is a **reserved indicator in YAML, which may not start a plain scalar**. The consequence was not a misread row but an **unparsable `cordis.patch.yml`** (`bad indentation of a mapping entry`), which killed the kernel child outright: **neither the installed app nor a dev run would start** until that one line was quoted by hand. Such ids are now quoted on write (the reader already strips quotes, so a toggle still recognises its own row), the market moves `0.2.0 → 0.2.1`, and a test pins the quoting, the `disabledIdsOf` round trip and the removal path
- **Dev launch refuses an installed kernel from another line**: the launcher used to warn and boot anyway, and the measured consequence was **two kernel lines polluting one profile** — the older kernel projects its own packages into the profile at boot (`node_modules/@scope/*` → `.dsh-module-fallback`), and every later boot of the followed line loads **that older batch** (`settings … this.load is not a function`, `configForms` absent, seven client entries pending). It now reads the version the installed kernel declares and stops when it differs from the line this repository follows, naming both line numbers and both ways forward (build a runtime, or point `DSH_APP_DEV_KERNEL=<dir>` at one); a same-line install still falls back as before
- **Kernel line moved to 0.1.7-alpha.2** (a patch-level move inside the same line; the followed spec is now `^0.1.7-alpha.2`, 17 manifests / 351 specifiers at once). That release lifts the vendor packages to its own versions (cordis **4.0.4**, schemastery **3.18.4**, loader 1.0.5, group 1.0.4, include 1.0.9, timer 1.1.6, cosmokit 1.8.5 …). **Two behaviour changes worth knowing**: ① the kernel child now fails loudly (`exit(1)`) on ANY uncaught rejection or exception, so an async throw inside a plugin is no longer swallowed; ② "first plugin install with no source configured" now looks for the best reachable npm source first (manual and private sources stay). **Nothing on our side needed a code change** — root suite 373/0, sixteen plugins 1226/0, `check:graph` clean. What remains to follow, and the verification list, are in `docs/kernel-0.1.7-alpha.1-regression.md` §12
...
- **Dev startup now follows a local runtime**: `npm run dev` no longer probes and runs `../deepseek-harness` straight from source (an unbuilt checkout boots the PREVIOUS build — measured on this machine: three builds behind HEAD, still carrying 0.1.6 symbols). The order is: the tree named by `DSH_APP_DEV_KERNEL`, then the live sources named by `DSH_APP_DEV_RUNTIME`, then **a runtime THIS repository built that declares the followed line** (`scratch/<name>/runtime`, decided by its manifest rather than by guessing at directories), and only then the installed runtime — **which it falls back to only on the same line** (see the second entry of this list)
