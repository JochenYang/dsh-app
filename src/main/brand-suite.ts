 ...
 *
 *      The links deliberately stay in this shared fallback even though the
 *      suite boots a profile of its own (suite-profile.ts): the fallback
 *      resolves from any profile — Node's parent walk reaches it from
 *      `profiles/<profile>` — and it is the one directory no profile's pnpm
 *      run prunes, while a profile-local `node_modules` belongs to the
 *      package manager the plugin market drives. Known boundary: a kernel
 *      packaged as pkg/asar (upstream's own desktop) skips the fallback
 *      wholesale; we spawn a plain node with a normal node_modules tree (link
 *      mode), which honours it. Revisit if we ever package the kernel.
 *
