...
/**
 * Which kernel a dev run boots, decided by what the run NAMES — never by what
 * happens to sit beside the repository.
 *
 * `DSH_APP_DEV_RUNTIME` names a deepseek-harness source checkout: the checkout
 * IS the kernel, its packages resolve per app, and the shell starts its built
 * `apps/desktop-host` with the machine's own Node.
 *
 * `DSH_APP_DEV_KERNEL` names an already-built runtime tree (`node/` + `app/`) —
 * the same shape a packaged install boots, without touching the installed one.
 *
 * Neither: the run follows the INSTALLED kernel, exactly like a packaged start.
 * That default is the point. The old one probed `../deepseek-harness` and ran
 * whatever build was in it, so a pulled-but-unbuilt checkout silently booted a
 * kernel nobody chose — measured on this tree: `apps/cli/lib` was three builds
 * behind HEAD and still carried 0.1.6 symbols. A dev run has to say something
 * about the artifact we ship, and it can only do that by booting one.
 */
const devCheckoutDir = (process.env.DSH_APP_DEV_RUNTIME ?? '').trim() || undefined
const devKernelDir = (process.env.DSH_APP_DEV_KERNEL ?? '').trim() || undefined
/** True when the kernel IS a source checkout — the only case that resolves per app. */
const kernelIsCheckout = isDev && devCheckoutDir !== undefined
/** True when this run boots a runtime tree it was pointed at, not the installed one. */
const kernelIsNamedRuntime = isDev && devCheckoutDir === undefined && devKernelDir !== undefined
/**
 ...
 * host starts, together with the profile they were judged against. They are the
 * one boot failure the shell cannot prevent (the kernel composes that file
 * itself), so they are kept here for the failure card to name — file, line and
 * row — and for the repair that installs what they ask for.
 */
let homeRepair: { readonly profileDir: string, readonly rows: readonly PatchSpecifier[] } = { profileDir: '', rows: [] }
/**
 * Set once the repair has answered "cannot": the CLI is not there, or nothing in
 * the rows survived the filter. The card then comes back WITHOUT the button,
 * instead of offering an action that cannot make progress.
 */
let installRepairUnavailable = false
...
async function offerStartupRecovery(message: string, detail?: string): Promise<void> {
  ...
  const rows = rowsDetail()
  if (rows !== '') lines.push(rows)
  const installable = homeRepair.rows
    .map((row) => row.specifier)
    .filter((specifier) => isPackageSpecifier(specifier))
  // A CHECKOUT run's most common failure by far is a stale workspace: the typert
  // artifacts are generated by its build, so a pulled-but-unbuilt checkout fails
  // every plugin registration and no amount of retrying can fix it. Name the
  // command instead of leaving the user with a bare exit code — a runtime-tree
  // run (packaged, or dev on an installed / named kernel) has no such failure
  // mode, and telling it to rebuild a checkout would send the user to the wrong fix.
  if (kernelIsCheckout) {
    lines.push(devCheckoutDir === undefined
      ? t('devMode.workspaceHint')
      : t('devMode.workspaceHintAt', { checkout: devCheckoutDir }))
  }
  const choice = await showStartupFailure(
    message,
    lines.join('\n'),
    installable.length > 0 && !installRepairUnavailable ? ['install-missing'] : [],
  )
  if (choice === 'retry') {
    app.relaunch()
    app.quit()
  } else if (choice === 'install-missing') {
    await installMissingPackages(installable)
...
/**
 * The home-layer rows this profile could not load, as the text both the failure
 * card and the give-up dialog show: the file, the line, the row, and what can be
 * done about it. Empty when there is nothing to say, which is every normal boot.
 */
function rowsDetail(): string {
  const rows = homeRepair.rows
  if (rows.length === 0) return ''
  const file = path.join(resolveDshHome(), PROFILE_PATCH_FILENAME)
  const lines = [t('serverFailure.homeRowsIntro', { file, profile: SUITE_PROFILE, count: rows.length })]
...
/** Whether a row's specifier is a package name, which is the only kind that can be installed. */
function isPackageSpecifier(specifier: string): boolean {
  return isInstallablePackageName(specifier)
}

/** One log line for the install repair (log-only, English, never rendered). */
function installLogLine(outcome: ProfileHealOutcome, packages: readonly string[]): string {
  const names = packages.join(', ')
  if (outcome.status === 'healed') {
    return `[suite-profile] installed the packages the home layer names and this profile lacked (${names})`
  }
  return `[suite-profile] could not install the packages the home layer names and this profile lacks (${names}): ${outcome.detail ?? 'unknown error'}`
}

/**
 * The repair for the home-layer rows this profile cannot load: install those
 * packages into the profile, then relaunch.
 *
 * The user's own configuration is not touched — the row they wrote stays exactly
 * as it is, and what changes is the profile it could not load in. That direction
 * is deliberate: the home layer applies to every profile of the install, so
 * editing it to fix one profile would break the others that row may be good for.
 */
async function installMissingPackages(packages: readonly string[]): Promise<void> {
  const bin = kernelCliPath()
  if (bin === '' || !existsSync(bin)) {
    // No CLI, no repair: the card comes back without the button, so it cannot be
    // clicked again for the same answer.
    installRepairUnavailable = true
    await offerStartupRecovery(t('serverFailure.summary', { advice: t('serverFailure.installNoCli') }))
    return
  }
  // The same progress line the boot uses, so the card gives way to a step the
  // user can watch instead of an unchanging failure.
  broadcastStatus({ phase: 'starting', message: t('status.installingMissing'), progress: null, step: 4 })
  const outcome = await installIntoProfile({
    profileDir: homeRepair.profileDir,
  ...
  if (outcome.status === 'healed') {
    logKernel(installLogLine(outcome, packages))
    app.relaunch()
    app.quit()
    return
  }
  if (outcome.status === 'ok') {
    // Nothing survived the filter, so nothing was run and nothing failed: the
    // card comes back with the rows and without a second offer.
    installRepairUnavailable = true
    await offerStartupRecovery(t('serverFailure.summary', { advice: t('serverFailure.other') }))
    return
  }
  logKernel(installLogLine(outcome, packages))
  await offerStartupRecovery(
    t('serverFailure.summary', { advice: t('serverFailure.other') }),
    t('serverFailure.installFailed', { detail: outcome.detail ?? 'unknown error' }),
  )
}
...
function hostRuntime(): {
  ...
  if (!kernelIsCheckout) {
    // ONE shape, two sources: the installed kernel a packaged start boots, or a
    // runtime tree a dev run named. A dev run also links the suite plugins in
    // from THIS repository, so its profile resolves outside the runtime tree by
    // construction — which is exactly what `allowLinkedProfile` permits.
    const dir = activeRuntimeDir()
  ...
  if (checkout === undefined) throw new Error(t('hostFailure.devHostMissing', { checkout: 'DSH_APP_DEV_RUNTIME' }))
...
async function reportStartupFailureAndExit(): Promise<void> {
  ...
  // The give-up dialog is often the ONLY thing a user ever sees: the card can be
  // replaced by the next status push while the automatic retry runs, and safe
  // mode does not help with a home-layer row (the kernel loads that file itself,
  // regardless of the suite rows). So the rows are repeated here, where they are
  // the last thing on screen.
  const homeRows = rowsDetail()
  if (kind === 'plugin-tree') {
    const choice = await promptThemedConfirm<'safe' | 'quit'>(
      mainWindow,
      {
        title: 'DSH APP',
        message,
        detail: homeRows === '' ? t('serverFailure.safeModeDetail') : `${homeRows}\n\n${t('serverFailure.safeModeDetail')}`,
        buttons: [
    ...
    const detail = homeRows === '' ? t('serverFailure.willExit', { message }) : `${homeRows}\n\n${t('serverFailure.willExit', { message })}`
    await promptNoticeThemed(mainWindow, 'error', 'DSH APP', detail)
  }
  app.quit()
}
 ...
 * fingerprint`). A packaged install runs the runtime's own binary, and so does a
 * dev run that boots one; only a source checkout — which ships no binary — falls
 * back to the machine's.
 */
function hostNodeBinary(): string {
  if (!kernelIsCheckout) {
    try {
      const executable = path.join(activeRuntimeDir(), 'node', NODE_BINARY_NAME)
...
/**
 * The runtime tree this run boots: the one a dev run named through
 * `DSH_APP_DEV_KERNEL`, else the installed kernel's active directory.
 *
 * Only meaningful when {@link kernelIsCheckout} is false — a checkout is not an
 * installed tree and has no `node/` or `app/` of its own.
 *
 * @returns the absolute runtime directory.
 * @throws when no kernel is active (see `KernelManager.getCurrentDir`).
 */
function activeRuntimeDir(): string {
  if (kernelIsNamedRuntime && devKernelDir !== undefined) return devKernelDir
 ...
 *
 * A checkout runs its own built CLI; a runtime tree — installed or named by a
 * dev run — carries it inside `app/`. Both the environment handed to the kernel
 * child (`DSH_APP_DSH_BIN`, which the market and the shell's own repair step
 * resolve) and the repair step itself ask this question, so they answer it the
 * same way.
 */
function kernelCliPath(): string {
  if (kernelIsCheckout && devCheckoutDir !== undefined) return path.join(devCheckoutDir, 'apps', 'cli', 'lib', 'bin.js')
  try {
...
async function startServerAndOpenWindow(): Promise<void> {
  ...
  const suiteRows = await prepareBrandSuite(
    isDev ? devSuiteSources() : prodSuiteSources(kernel.getCurrentDir()),
    {
      ...
      // and comments out rows the host could load — measured: three of them in
      // one home layer, which took the user's preset away.
      kernelNodeModules: path.join(kernel.getCurrentDir(), 'app', 'node_modules'),
      report: logKernel,
    },
  )
  ...
  // what it can do is remember them, say so on the failure card, and offer to
  // install what they ask for.
  homeRepair = { profileDir: profile.dir, rows: suiteRows.homeUnloadable }
  ...
  // The injection itself is the old rule, unchanged: a detected local proxy is
  // handed to the child, and nothing here inspects the resolver. What made the
  // provider calls fail was never the proxy — it was the `undici` version the
  // runtime resolved (see the pin in scripts/build-runtime.mjs), and the kernel
  // needs the proxy env for web_fetch's address validation on a TUN network.
  // DSH_APP_PROXY_INJECT=off keeps a machine's environment free of it.
  const injectOff = (process.env.DSH_APP_PROXY_INJECT ?? '').trim().toLowerCase() === 'off'
  const { env: proxyEnv, injected } = withDetectedProxy(scrubbed.env, injectOff ? undefined : detectedProxy)
...
async function handleServerDown(reason: string): Promise<void> {
  ...
  if (restartAttempts >= 2 && !isDev) {
    // Before the line steps back, find out what stepping back HIDES. A kernel
    // older than the one that wrote them does not list V4 session logs at all
    // (see `session-logs.ts`), so the list comes back shorter than the user left
    // it — with nothing on disk actually gone. The count only chooses the
    // sentence; the rollback happens either way, because a window that cannot
    // load is worse than a list that needs explaining.
    const hiddenSessions = await countV4SessionLogs(path.join(resolveDshHome(), 'sessions'))
    ...
    if (rolledBack) {
      ...
      void promptNoticeThemed(mainWindow, 'warning', 'DSH APP', hiddenSessions > 0
        ? t('kernelUpdate.rollbackHidesNewerSessions', { version: rolledBack.manifest.dshVersion, count: hiddenSessions })
        : t('kernelUpdate.rollbackBootFailed', { version: rolledBack.manifest.dshVersion }))
      await startServerAndOpenWindow()
      return
    }
...
async function installBundledKernel(): Promise<void> {
  // The bundled runtime is this build's own payload, so every kind of run can
  // install it: the old `isDev ? null :` made a dev run skip straight to the
  // network even though a perfectly good runtime was sitting in its resources.
  const files = bundledTarball(bundledKernel)
  if (files === null) {
    // Nothing bundled. A packaged build goes to the network for one; a dev run
    // does not — it boots the kernel the developer pointed at, and silently
    // downloading one nobody chose is what this whole change removes. The message
    // lands on the failure card (see the `whenReady().then(boot).catch(...)`).
    if (isDev) throw new Error(t('kernel.devKernelMissing'))
    await installKernel()
    return
  }
  try {
...
async function boot(): Promise<void> {
  ...
  kernel = new KernelManager({
    ...
    arch: process.arch,
    // The kernel is a source checkout only when a dev run NAMED one; every other
    // run — packaged or dev — boots a runtime tree (the installed one, or the one
    // `DSH_APP_DEV_KERNEL` points at). See the resolution block at the top.
    source: kernelIsCheckout ? 'dev' : 'artifact',
    channel,
    devCheckoutDir,
    localRuntimeDir: kernelIsNamedRuntime ? devKernelDir : undefined,
    artifactOwner,
    artifactRepo,
  ...
  const shellActions = createShellActionHandler({
    ...
    officePayload,
    configCheck: () => {
      // The kernel CLI and its own Node, resolved the same way the boot uses
      // them: `kernelCliPath` knows the dev-checkout case, and `hostNodeBinary`
      // guarantees a real Node rather than Electron's own.
      const bin = kernelCliPath()
      if (bin === '' || !existsSync(bin)) return Promise.reject(new Error('no kernel CLI in this shell'))
      return checkConfigSchema({
        bin,
        node: hostNodeBinary(),
        profileName: SUITE_PROFILE,
        dshHome: resolveDshHome(),
        // The scope every suite plugin publishes under, taken from the module
        // that owns it rather than spelled again here.
...
if (!gotLock) {
  ...
  // `boot` is the whole startup path. An error escaping it used to become an
  // unhandled rejection with the window parked on the loading page and nothing
  // on screen to explain it; routing it to the failure card turns any such
  // escape — a dev run with no kernel to boot, an install that died before the
  // server path was even reached — into a surface that names the reason.
  void app.whenReady().then(boot).catch((error: unknown) => {
    const detail = error instanceof Error ? error.message : String(error)
    console.error('[boot] failed:', detail)
    void offerStartupRecovery(t('serverFailure.summary', { advice: detail }))
  })
