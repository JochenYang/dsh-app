...
const AUDIT = [
  ...
  },
  {
    match: 'test/*.test.mjs',
    count: 19,
    why: 'root suites: every call removes an os.tmpdir scratch root that test just created (the mirror test also deletes a hardlink-only profile mirror on purpose) — no link out of any of them',
  },
  {
    match: 'plugins/*/tests/**',
    count: 97,
    why: 'plugin suites: every call removes an os.tmpdir scratch root or fixture home that test just created — no link points out of any of them',
  },
]
