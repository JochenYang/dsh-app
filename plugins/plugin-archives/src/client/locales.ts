...
export const zh = {
  ...
  'archives.stale.hint': '归档记录仍在，但其会话日志已不在磁盘上；清理只会移除这些无效记录',
  ...
  'archives.confirm.delete': '即将删除{label}（约 {bytes}），删除后不可恢复。',
...
export const en: Record<ArchivesKey, string> = {
  ...
  'archives.stale.hint': 'The archive records are still here, but their session logs are gone from disk; pruning removes only these stale records',
  ...
  'archives.confirm.delete': 'About to delete {label} (about {bytes}); this cannot be undone.',
