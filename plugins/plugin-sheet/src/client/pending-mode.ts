 ...
 * The mode is persisted per session, so a hero toggle has nothing to write to
 ...
 *
 * @module @dsh-app/plugin-sheet/client/pending-mode
 */

/** Parked toggle: `undefined` means nothing is parked. */
...
export const pendingMode = {
  /** The parked toggle, or `undefined` when the slot is empty. */
  get(): PendingMode { return parked },
  /** Park a toggle for the next session-bound occurrence. */
  set(enabled: boolean): void {
    parked = enabled
    emit()
  },
  /** Take the parked toggle, emptying the slot; `undefined` when it was empty. */
  consume(): PendingMode {
    const value = parked
