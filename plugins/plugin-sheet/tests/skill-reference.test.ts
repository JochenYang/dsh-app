 ...
 * browser.
 *
 * @module @dsh-app/plugin-sheet/tests/skill-reference
 */
...
test('skill reference: constants mirror the installed skill name', () => {
  assert.equal(SKILL_NAME, 'dsh-sheet')
  assert.equal(PREFILL_SKILL, SKILL_NAME)
  assert.equal(SKILL_TOKEN, `/${SKILL_NAME}`)
  assert.equal(REFERENCE_SOURCE, 'dsh-office-sheet')
  assert.equal(CHIP_LABEL, 'Excel')
...
test('skill text: the delivery step registers the artifact with present', () => {
  ...
  // artifact with node:fs — without this instruction the rendered .xlsx reaches
...
test('skill reference: the built client artifact carries the source and no chip skin', () => {
  ...
  assert.match(bundle, /dsh-office-sheet/)
...
test('chip cleanup: the office token regex removes head, mid-text, trailing and coexisting tokens', () => {
  ...
  assert.equal(strip('材料 /dsh-word'), '材料 ', 'a token at the very end still goes, its leading space stays')
  ...
  assert.equal(strip('/dsh-sheetX '), '/dsh-sheetX ')
  assert.equal(strip('比值 1/2'), '比值 1/2')
})

test('chip cleanup: stripSkillToken removes only the named token', () => {
  assert.deepEqual(stripSkillToken('/dsh-sheet /dsh-pdf 材料', '/dsh-sheet'), { text: '/dsh-pdf 材料', removed: true })
  assert.deepEqual(stripSkillToken('材料', '/dsh-sheet'), { text: '材料', removed: false })
})

test('chip cleanup: turning another format on replaces the existing office chip', () => {
  const written: string[] = []
  const calls: unknown[] = []
  const state = { draft: '/dsh-ppt 汇总表格', phase: 'plain', draftRev: 3 }
  const binding = bindingOf(
...
test('chip cleanup: without a pipeline the cleaned draft gets the literal token back', () => {
  const written: string[] = []
  const state = { draft: '/dsh-pdf ', phase: 'plain', draftRev: 2 }
  const binding = bindingOf(
    { setDraft: text => { written.push(text); state.draft = text; state.draftRev += 1 } },
    state,
  )
...
test('chip cleanup: the stand-down path removes only this plugin chip', () => {
  const written: string[] = []
  const state = { draft: '/dsh-sheet /dsh-ppt 材料', phase: 'plain', draftRev: 1 }
  const binding = bindingOf(
    { setDraft: text => { written.push(text); state.draft = text } },
    state,
  )
  assert.equal(removeSkillReference(binding), true)
  assert.deepEqual(written, ['/dsh-ppt 材料'])
