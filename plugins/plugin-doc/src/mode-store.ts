/**
 * Session-level Word-mode persistence.
 *
 * One JSON file (`<storeDir>/mode.json`) maps sessionId → { enabled,
 * updatedAt }. Word mode has no template, so the state is a single boolean;
 * unlike the PPT store there is no legacy shape to migrate. The in-memory map
 ...
 *
 * @module @dsh-app/plugin-doc/mode-store
 */
...
/** One persisted entry: whether the session's Word mode is on. */
...
export class DocModeStore {
  ...
  /** Whether the session has Word mode on. */
  ...
  /**
   * Turn Word mode on or off. The map updates synchronously so the next prompt
