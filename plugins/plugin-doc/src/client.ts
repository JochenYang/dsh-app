/**
 * DSH APP Word suite — client half.
 *
 ...
 *
 * @module @dsh-app/plugin-doc/client
 */
...
import { mountWordOfficeBar } from './client/office-entry.tsx'
...
import { en as docEn, NS as DOC_NS, zh as docZh } from './client/locales.ts'
import type { DocKey } from './client/locales.ts'
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface LocaleNamespaceMap {
    /** The Word capsule's copy (one namespace per plugin, one owner per namespace). */
    [DOC_NS]: DocKey
  }
}
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(
    () => ctx.locale.register(DOC_NS, { zh: docZh, en: docEn }),
    'dsh-app plugin-doc: dictionaries',
  )
  ...
  ctx.effect(
    () => mountWordOfficeBar(ctx.uiSession.adapter.current, ctx.locale),
    'dsh-app plugin-doc: office-bar capsule',
  )
  ...
  ctx.inject(['inputTriggers'], (scope) => {
    ...
    scope.effect(
      ...
      'dsh-app plugin-doc: skill reference source',
    )
  })
}
