...
> 用于**检查和修改已有的** OOXML 文件；本技能用于**从结构化 JSON 工程产出新的** .docx
> （\`doc_write\` → \`doc_check\` → \`doc_render\`）。手边是要改的文件时用内核技能，要交一份排版规范的
> 成品时用本技能——两者都能读写 .docx，混用会把"改一个现有文件"变成"重排一遍"。
...
5. 校验通过后调用 \`doc_render\`（入参 \`file_path\` 与新的 \`output_file\`，\`.docx\` 结尾）。渲染内部仍会复验：\`status: needs_revision\` 表示未导出任何文件，回到第 4 步继续修；\`status: exported\` 才算交付。导出成功后，**若当前会话提供 \`present\` 工具，就用它把成品登记为本次交付**（\`files: [{ "path": "<产出>.docx" }]\`；相对路径按会话工作目录解析，文件必须已经存在）——登记过的文件才会出现在会话的交付卡片里，用户能直接点开；没有该工具时跳过这一步即可。最后在回复结尾给出产出 \`.docx\` 的**明确路径引用**与 \`.doc.json\` 工程路径（工程文件一般不必登记，除非用户明确要它），并说明可用你本机的 Word 打开继续编辑；不声称已在本机打开验证过。
