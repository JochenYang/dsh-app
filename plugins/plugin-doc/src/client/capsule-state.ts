/**
 * Presentation state of the Word capsule, kept DOM- and React-free so the
 ...
 * externals.
 *
 * Word mode has no template, so the capsule is a plain toggle. The click target
 ...
 *
 * @module @dsh-app/plugin-doc/client/capsule-state
 */
...
/** One Word mode as the capsule reads it, before or after the session load. */
export interface CapsuleMode {
  /** Whether Word mode is on. */
...
export interface CapsuleState {
  /** Whether Word mode is on. */
  readonly enabled: boolean
  /** Capsule text: always the bare label (Word has no template). */
