/**
 * The office-suite capsule bar, as this plugin implements it: one container
 * every format plugin in the suite mounts into, anchored directly after the
 * composer card.
 *
 * The bar is a single row of capsule hosts inserted after
 * `div[data-composer-card]` (the InputBar card in ui-conversation). That
 * landmark is what makes the row work in both the new-session hero and an open
 * session without claiming a seat: the hero seats are all singles the shipped
 * UI already fills, and the session-scoped seats need a session the hero does
 * not have. The card is React-rendered by a root that re-renders on every
 * keystroke, so placement is reconciled (one pass per animation frame) instead
 * of written once.
 *
 * This is a standalone implementation of the suite's DOM contract — the plugin
 * must not import its siblings, and the contract is small enough to restate:
 * the container class `dshOfficeBar`, one host per format as
 * `div.dshOfficeFormat[data-office-format="<fmt>"]`, formats ordered by
 * {@link OFFICE_FORMAT_ORDER}, hosts deduplicated by that attribute, and the
 * container created by whichever contributor arrives first. Any number of
 * plugins therefore converge on one bar regardless of load order, and an
 * unloaded plugin leaves no residue.
 *
 ...
 * document mutations.
 *
 * @module @dsh-app/plugin-doc/client/office-bar
 */
...
/** The format this plugin contributes. */
...
/**
 * Reorder the bar's hosts into {@link orderOfficeFormats}. Writes only when the
 * current order differs, so a pass that has nothing to fix stays silent.
 */
 ...
 * composer and coming back costs no remount. Passes are coalesced per animation
 * frame, and {@link officeBarPlacement} is what keeps a pass from re-inserting
 * what it just inserted, so the observer cannot feed itself.
 *
