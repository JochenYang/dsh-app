/**
 * Session Word-mode persistence: the on-disk round trip, the "only an explicit
 ...
 *
 * @module @dsh-app/plugin-doc/tests/mode
 */
...
import { DocModeStore } from '../src/mode-store.ts'

function withTempStore(run: (file: string) => Promise<void>): Promise<void> {
  const dir = mkdtempSync(join(tmpdir(), 'docd-mode-'))
...
test('mode: a missing file starts empty and unknown sessions read as off', async () => {
  await withTempStore(async (file) => {
    const store = new DocModeStore(file)
    store.load()
...
test('mode: a toggle round-trips through disk and reads back', async () => {
  await withTempStore(async (file) => {
    ...
    const reloaded = new DocModeStore(file)
    reloaded.load()
    ...
    const cleared = new DocModeStore(file)
    cleared.load()
...
test('mode: corrupt content degrades to empty instead of throwing', async () => {
  await withTempStore(async (file) => {
    ...
    const store = new DocModeStore(file, { warn: message => { warnings.push(message) }, info: () => {} })
    store.load()
    assert.equal(store.isEnabled('s1'), false)
