...
export const zh = {
  ...
  'presets.backup.intro': '导出或恢复当前配置：备份包含模型与 provider 设置（settings.yaml）、home 层补丁（cordis.patch.yml）、凭据库（.credentials.yaml，内含 API 密钥明文）、AGENTS.md、插件配置、profile 补丁层与 hooks 脚本。导出前会扫描常见密钥形态，命中的文件会列在导出结果里提醒——备份内含明文密钥，请当作私有文件：只在本机保管，或先加密再传输。包内含 provider 地址、请求头与会在本机执行的 hook 脚本，且只应导入自己导出的备份——它决定请求去向、免审批范围与本机执行的命令。 依赖清单中的本地 file: 路径会按原样恢复，换机器导入可能失效。',
  ...
  'presets.backup.export.done': '已导出 {file}',
  'presets.backup.export.secrets': '注意：备份内含疑似凭据内容——{list}。ZIP 中的密钥为明文，请只在本机保管，或先加密再传输；换机器导入后如需吊销，请轮换这些密钥。',
  'presets.backup.restore.written': '已恢复 {written} 项配置',
  'presets.backup.restore.unchanged': '，{unchanged} 项无变化跳过',
  'presets.backup.restore.backedUp': '，补丁层、settings.yaml 与凭据库已自动备份',
  ...
  'presets.backupOverwrite.message': '备份中的以下文件与本机当前配置不同，覆盖导入将替换它们（其中补丁层、settings.yaml 与凭据库会先自动备份，其余文件直接替换），且无法撤销：{files}',
  'presets.backup.file.credentials': '凭据库（API 密钥）',
  ...
  'presets.host.backupSecretContent': '配置文件「{rel}」命中疑似凭据内容（规则 {rule}）',
...
export const en: Record<PresetsKey, string> = {
  ...
  'presets.backup.intro': 'Export or restore the current configuration: the backup carries model and provider settings (settings.yaml), the home patch layer (cordis.patch.yml), the credential store (.credentials.yaml, API keys in plain text), AGENTS.md, plugin configs, the profile patch layer, and hook scripts. Common credential shapes are scanned before export and every matching file is listed in the export result — the backup holds plaintext keys, so treat it as a private file: keep it local, or encrypt it before transferring it. The package holds provider endpoints, request headers, and hook scripts that run on this machine, and only a backup you exported yourself should ever be imported — it decides where requests go, what runs without approval, and which commands execute locally. Local file: paths in the dependency list are restored as-is, so importing on another machine may not work.',
  ...
  'presets.backup.export.done': 'Exported {file}',
  'presets.backup.export.secrets': 'Note: the backup contains credential-like content — {list}. The keys inside the zip are plaintext: keep the file local, or encrypt it before transferring; rotate these keys if the archive ever leaks.',
  ...
  'presets.backup.restore.backedUp': ', the patch layer, settings.yaml and the credential store were backed up first',
  ...
  'presets.backupOverwrite.message': 'These files in the backup differ from the current configuration; importing with overwrite replaces them (of which the patch layer, settings.yaml and the credential store are backed up first, the rest are replaced outright) and cannot be undone: {files}',
  'presets.backup.file.credentials': 'the credential store (API keys)',
  ...
  'presets.host.backupSecretContent': 'configuration file "{rel}" matched a credential-like pattern (rule {rule})',
