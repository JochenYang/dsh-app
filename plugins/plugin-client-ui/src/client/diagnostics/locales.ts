...
export const zh = {
  ...
  'diag.payload.updateAvailable': '已安装 v{installed}，有新版本',
  ...
  'diag.payload.hintUpdate': '内核自带的转换组件版本变了，当前内核需要 v{version}。已安装的 v{installed} 是完整的，但它自带的引擎与当前内核要求的不一致，转换会失败。点下面的按钮更新到需要的版本，装好后直接重试转换即可，不需要重启应用。',
  ...
  'diag.check.title': '组合静态检查',
  'diag.check.action': '运行检查',
  'diag.check.running': '检查中…',
  'diag.check.idle': '未运行',
  'diag.check.noReport': '内核没有返回检查结果。',
...
export const en: Record<DiagnosticsKey, string> = {
  ...
  'diag.payload.updateAvailable': 'Installed v{installed} — update available',
  ...
  'diag.payload.hintUpdate': 'The conversion components this kernel ships moved to a new version, and it needs v{version}. The v{installed} you have is complete, but the engine it carries is not the one this kernel asks for, so conversions fail. Use the button below to update to the required version, then retry the conversion — no restart needed.',
  ...
  'diag.check.title': 'Static composition check',
  'diag.check.action': 'Run check',
  'diag.check.running': 'Checking…',
  'diag.check.idle': 'Not run',
  'diag.check.noReport': 'The kernel returned no check result.',
