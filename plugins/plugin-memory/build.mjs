#!/usr/bin/env node
/**
 * Builds the memory plugin's two halves:
 *   lib/index.js  — the host half (Node): memory store, LLM tools,
 *     system-prompt injection, and settings routes. Bundled with
 *     @deepseek-ai/* and cordis EXTERNAL (they resolve from the profile's
 *     node_modules at load time), like the client half below.
 *   lib/client.js — the browser half (settings-page section), wrapped in
 *     the window.__ModuleLoader__.load({ id, factory }) closure the dsh
 *     web client loader expects (same shape as plugin-usage's build).
 *
 * Run from the dsh-app root (esbuild resolves out of dsh-app/node_modules):
 *   node plugins/plugin-memory/build.mjs
 */
...
const ID = '@dsh-app/plugin-memory'
