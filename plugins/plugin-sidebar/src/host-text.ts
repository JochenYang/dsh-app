/**
 * The coded wire message this plugin's git routes answer with.
 *
 * One definition for both sides of the contract: the routes BUILD these, and
 * the Git tab renders them through its dictionary (`sidebar.fail.*`). The tab
 * mirrors the shape structurally — a suite plugin bundles standalone and never
 * imports another plugin's source — so this module is the host's own record of
 * the wire shape.
 *
 * @module @dsh-app/plugin-sidebar/host-text
 */
