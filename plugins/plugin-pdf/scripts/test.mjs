 ...
 *
 * Run from anywhere:
 *   node plugins/plugin-pdf/scripts/test.mjs
...
buildSync({
  ...
  // The bundled CJS dependencies (pdf-lib → pako, and PDF.js) call require()
  // for node builtins at render time; ESM output has no require, so hand one
  // over — the same fix the host build applies post-build.
  banner: {
