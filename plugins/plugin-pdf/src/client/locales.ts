/**
 * Dictionary of the PDF capsule, in the plugin's own locale namespace.
 *
 ...
 * copy; {@link PdfKey} is its key union and the English dictionary is typed with
 ...
 *
 * @module @dsh-app/plugin-pdf/client/locales
 */

/** Locale namespace owned by this plugin's client half. */
export const NS = 'dsh-app.pdf'

/** Simplified Chinese dictionary — the pre-i18n capsule copy, verbatim. */
export const zh = {
  'capsule.label': 'PDF',
  'capsule.hintOn': 'PDF 模式已开启；点击关闭',
  'capsule.hintOffPending': '点击开启 PDF 模式，将在会话开始时应用',
  'capsule.hintOff': '点击开启 PDF 模式',
  'capsule.pendingNotice': '将在会话开始后生效',
  'capsule.skillHint': '已开启 {label} 模式；技能引用未能放入输入框，可手动输入 {token}',
} as const

/** English dictionary, key-identical to the Chinese source of truth. */
export const en: Record<PdfKey, string> = {
  'capsule.label': 'PDF',
  'capsule.hintOn': 'PDF mode is on; click to turn it off',
  'capsule.hintOffPending': 'Click to turn on PDF mode; it applies when the session starts',
  'capsule.hintOff': 'Click to turn on PDF mode',
...
/** Key domain of the `dsh-app.pdf` namespace (zh is the source of truth). */
export type PdfKey = keyof typeof zh
