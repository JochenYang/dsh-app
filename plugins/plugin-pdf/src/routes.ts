/**
 * PDF-mode API under `/plugins/@dsh-app/plugin-pdf/api`:
 ...
 */
export function registerPdfRoutes(webServer: WebServerLike, store: PdfModeStore, activeFile: string): () => void {
  const disposers = [
    webServer.register({
      kind: 'exact',
      ...
      handler: (req, res) => {
        ...
        if (req.method === 'GET') {
          ...
          ok(res, { enabled: store.isEnabled(sessionId), updatedAt: store.updatedAtOf(sessionId) })
          return
        }
        if (req.method !== 'PUT') {
