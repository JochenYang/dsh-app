 ...
 * browser.
 *
 * @module @dsh-app/plugin-ppt/tests/skill-reference
 */
...
test('skill reference: constants mirror the installed skill name', () => {
  assert.equal(SKILL_NAME, 'dsh-ppt')
  assert.equal(PREFILL_SKILL, SKILL_NAME)
  assert.equal(SKILL_TOKEN, `/${SKILL_NAME}`)
  assert.equal(REFERENCE_SOURCE, 'dsh-office-ppt')
  assert.equal(CHIP_LABEL, 'PPT')
...
test('skill reference: the built client artifact carries the source and no chip skin', () => {
  ...
  assert.match(bundle, /dsh-office-ppt/)
...
test('chip cleanup: the office token regex removes head, mid-text, trailing and coexisting tokens', () => {
  ...
  assert.equal(strip('/dsh-ppt '), '')
  assert.equal(strip('/dsh-word 材料'), '材料', 'a head token and its separator go together')
  ...
  // Non-office text, a longer identifier and a bare slash untouched.
...
test('chip cleanup: stripSkillToken removes only the named token', () => {
  assert.deepEqual(stripSkillToken('/dsh-ppt /dsh-pdf 材料', '/dsh-ppt'), { text: '/dsh-pdf 材料', removed: true })
  assert.deepEqual(stripSkillToken('材料', '/dsh-ppt'), { text: '材料', removed: false })
})
...
test('chip cleanup: turning another format on replaces the existing office chip', () => {
  ...
  const decision = applySkillReference(binding, true)
  assert.equal(decision.kind, 'insert')
  // The foreign chip is stripped once, then the chip is placed against the
  // post-write revision.
...
test('chip cleanup: without a pipeline the cleaned draft gets the literal token back', () => {
  const written: string[] = []
  const state = { draft: '/dsh-word ', phase: 'plain', draftRev: 2 }
  const binding = bindingOf(
    { setDraft: text => { written.push(text); state.draft = text; state.draftRev += 1 } },
    state,
  )
  // No pipeline: the cleanup writes the emptied draft, then the seed adds ours.
  const decision = applySkillReference(binding, true)
  assert.deepEqual(decision, { kind: 'seed', draft: `${SKILL_TOKEN} ` })
  assert.deepEqual(written, ['', `${SKILL_TOKEN} `])
})
...
test('chip cleanup: the stand-down path removes only this plugin chip', () => {
  const written: string[] = []
  const state = { draft: '/dsh-ppt /dsh-pdf 材料', phase: 'plain', draftRev: 1 }
  const binding = bindingOf(
  ...
  // Nothing of ours left: a repeat is a no-op.
  assert.equal(removeSkillReference(binding), false)
  assert.equal(written.length, 1)
})

test('skill text: the two authoring channels share one format and one gate set', () => {
  assert.ok(SKILL_MARKDOWN.includes('两种创作通道'))
  assert.ok(SKILL_MARKDOWN.includes('模板模式'))
  assert.ok(SKILL_MARKDOWN.includes('自由模式'))
