...
2. **诊断行要自己的图标**。上游按 section id 映射图标，其余一律回退到通用齿轮；「模型高级设置」已有立方体字形，
   ...
   —— 这一字形现在是**「维护设置」行**的图标（见下面的合并说明与表格），诊断不再是单独一行。
...
| 19    | 并行子代理   | 本套件                 |
| 20    | Agent 预设   | 上游                   |
| 21    | 用量统计     | 本套件（本轮从「维护设置」的页签提为独立行） |
| 22    | 会话记忆     | 本套件（本轮随「维护设置」后移） |
| 23    | 会话归档     | 本套件（同上）          |
| 24    | 维护设置     | 本套件（本轮从 22 后移，排在套件全部行之后） |
| 25    | 已归档会话   | 上游（钉死，动不了）      |

原先三行各自独立的页面已合并成一行「维护设置」（order 24）：它是**一个分区、两个页签**——预设包（2）、
诊断（3）。页签槽位 `settings.dsh-app-maintenance.tab`（root / list）由分区所有者 `plugin-client-ui` 在注册分区那次调用里声明，
另一个插件往里注册；容器画页签条、挂载选中的那个页签，**选过的页签保持挂载**（切回来不丢状态），
只有单个贡献时直接当页面本身渲染——这套写法照抄内核自己的「内置插件」分区，没有另造一套。
**2026-09-24 变更**：第三个页签「用量统计」**提为独立行（order 21，紧跟 20 之后）**——
理由是语义：留在「维护设置」的是**系统级**页面（预设包、诊断，回答"这套装是否健康"），而用量是**用户自己的数据视图**
（回答"我用了多少"），藏在页签后面难以发现（实测：在侧栏找「用量统计」找不到）。它同时也与「账号与余额」相邻：
账号页显示**服务端**余额，用量页显示**本地会话日志**折算的消耗，两者并置才读得通。提出后 plugin-usage 不再是页签贡献者，
三份逐字相同的槽位声明降为两份（`plugin-client-ui` 与 `plugin-presets`），
`plugin-client-ui/tests/settings-merge.test.ts` 增一条断言钉住"plugin-usage 不再携带该块"。

**分带理由**：11–14 = 模型与集成配置；15–19 = 生态与运行（插件 → 子代理）；20 = Agent 预设；
21 = 用量统计（用户自己的数据视图，紧邻 Agent 预设之后）；
24 = 维护设置（套件自身的两个系统级页面收成一个带页签的入口，**排在套件全部行之后**：设置里的「维护」就该是最后一项，它此前夹在会话数据中间，读起来是「数据 → 系统 → 数据」的来回跳。本机 order 300 的第三方行仍在它之后）；
23–25 = 会话数据三连（记忆 → 归档 → 已归档，把原本隔开的两个归档页凑到一起）。
...
**合并后的补充判定**（同一条探针，合并那一轮加，本轮随「用量统计」提行更新）：轨道只有一行「维护设置」且原三行消失、这一行自带图标、
**「用量统计」是自己的轨道行，且「维护设置」排在套件全部行之后**、页签条恰好两个且顺序为
预设包 → 诊断、只有一个页签被选中且只有它留在 tab 顺序里、选中的页签对应一个可见页签面板，
以及**切到第二个页签后第一个仍挂载**（`hidden` 而已，状态不丢）；`--sweep` 与 `--section 维护设置` 会依次点开三个页签分别统计残留汉字，
所以原本单独成行的诊断页仍在扫。探针链接套件插件到**两个** scope（照 `src/main/brand-suite.ts`）——只链共享 fallback 时，
内核的 profile 解析器不认套件包，十七个条目会全部以「Cannot find package」激活失败。
