...
2. **诊断行要自己的图标**。上游按 section id 映射图标，其余一律回退到通用齿轮；「模型高级设置」已有立方体字形，
   ...
   —— 这一字形现在是**「维护」行**的图标（见下面的合并说明与表格），诊断不再是单独一行。
...
原先三行各自独立的页面已合并成一行「维护」（order 22）：它是**一个分区、三个页签**——用量统计（1）、预设包（2）、
诊断（3）。页签槽位 `settings.dsh-app-maintenance.tab`（root / list）由分区所有者 `plugin-client-ui` 在注册分区那次调用里声明，
另外两个插件往里注册；容器画页签条、挂载选中的那个页签，**选过的页签保持挂载**（切回来不丢状态），
只有单个贡献时直接当页面本身渲染——这套写法照抄内核自己的「内置插件」分区，没有另造一套。于是原先的 16、21 两行随之消失。

**分带理由**：11–14 = 模型与集成配置；15–19 = 生态与运行（插件 → 子代理）；20 = Agent 预设；
22 = 维护（套件自身的三个页面收成一个带页签的入口，仍在系统级的位置、会话数据之前）；
...
**合并后的补充判定**（同一条探针，合并那一轮加）：轨道只有一行「维护」且原三行消失、这一行自带图标、页签条恰好三个且顺序为
用量统计 → 预设包 → 诊断、只有一个页签被选中且只有它留在 tab 顺序里、选中的页签对应一个可见页签面板，
以及**切到第二个页签后第一个仍挂载**（`hidden` 而已，状态不丢）；`--sweep` 与 `--section 维护` 会依次点开三个页签分别统计残留汉字，
