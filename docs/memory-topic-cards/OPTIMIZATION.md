...
- 外部调研只保留可独立成立的工程结论：本文不记录调研对象的源码路径与原始摘录，读者无需访问外部仓库即可理解每条要求的理由。
- 语言：与同目录 `DESIGN.md` 一致使用中文。代码注释与 JSDoc 仍按仓库约定用英文。

## 1. 调研判定表

对一款同类终端 agent 产品的记忆系统做了完整调研（存储模型、捕获、整合、注入、检索五个面）。下表是**逐条判定**，含否决与观望——否决项同样入库，否则下一轮会重复讨论。
...
**为何现在不做**。对标实现的索引条目比我们短得多，因为它的笔记正文可以很长，注入全文不可行；而我们的卡正文上限 400 字符（`memory-store.ts:54`），全文注入的边际成本本来就低。**触发条件应与实测注入量挂钩，而不是卡数**。
...
| 不做 | 理由 |
|---|---|
| 引入 SQLite 状态库 / 租约 / 操作状态机 / 计划哈希 / 孤儿收养 | 对标实现用约 1750 行撑起这套机制，服务的是"每轮捕获 512 个观察"的量级；我们的规模是每作用域几十张卡，且 `DESIGN.md` §0 已判定该量级下索引 + 相似度规则足够。引入状态库意味着新依赖、新迁移、新崩溃恢复测试面。**借其两点即可**：内容哈希做身份（A/C 已用）、拒绝而非静默（A/C/D）。 |
| 每轮对话结束调模型做捕获 | 现有 60s 静默窗口 + `MIN_NEW_MESSAGES` + `MIN_NEW_CHARS` 双门控（`distiller.ts:60,146,156`）在"不打断用户"上更优；每轮调用是数据驱动的产品取舍，不是我们的目标。 |
| 用普通文件工具替代语义化记忆工具 | 对标实现依赖一个成熟的、带策略层的文件工具桥。我们的 `memory_save` / `memory_recall` / `memory_forget` 让模型不必理解目录布局，写入路径收敛为两条（工具 + distiller），更易校验。 |
...
| 日期 | 变更 |
|---|---|
| 2026-09-19 | 初稿。调研判定表、A–G 方案、回归台账骨架 |
