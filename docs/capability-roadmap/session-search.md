...
1. **overlay 两行**（`dsh-app.patch.yml`）：
   ```yaml
   ...
   ```
   （第一行覆写 base 行 config；id 对齐 base 的行 id，last-write-wins。）
2. **agent 能力**：`tool-session-query` 挂载后，模型获得"检索历史会话"工具——
   这是记忆之外的第二条持久知识通道（记忆=策展后的条目；检索=原始对话）。
...
4. tool-session-query 在 standard preset 会话内可调用（V1 探针确认 preset 层挂载方式：
   host 行 vs 需入 preset 域——参考 web bundle 把工具行移到 preset 的注释）。
